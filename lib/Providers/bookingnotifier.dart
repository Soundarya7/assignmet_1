...
import 'package:assignmet_1/models/bookingstate.dart';
...
class bookingNotifier extends StateNotifier<AsyncValue<List<BookingState>>> {
  bookingNotifier() : super((AsyncValue.loading()));
   ...
   Future<List<BookingState>> fetchBookedDates(BuildContext context,  Propertystate property,WidgetRef ref) async {
      ...
      print("Response: ${booked_dates}");
       if (response.statusCode == 200) {
        ...
        if (booked_dates.isNotEmpty) {
          ...
          final reviews = booked_dates.map((json) => BookingState.fromJson(json)).toList();
          state = AsyncValue.data(reviews);
           
           return booked_dates.map((json) => BookingState.fromJson(json)).toList();
        }
       else {
 ...
 Future<void> Bookproperties(BuildContext context,  Propertystate property,String? date,WidgetRef ref) async {
   final prefs = await SharedPreferences.getInstance();
    final extractData =json.decode(prefs.getString('userData')!) as Map<String, dynamic>;
    String token = extractData['token'];
    ...
    headers: {
     ...
     print("Bookproperties HTTP Response: ${response.statusCode} - ${response.body}");
     var booked_dates = json.decode(response.body);
      print("Bookproperties Response: ${booked_dates}");
       ...
       showDialog(
        ...
        print("Bookproperties Failed response: ${response.statusCode} - ${response.body}");
        throw Exception('Failed to load properties');
      }
    } catch (e) {
...
final bookingProvider = StateNotifierProvider<bookingNotifier, AsyncValue<List<BookingState>>>((ref) {
