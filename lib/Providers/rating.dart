...
var response = await http.post(Uri.parse(url),
    ...
    Future<List<RatingState>>getreviews(BuildContext context,  Propertystate property,WidgetRef ref) async {
         final prefs = await SharedPreferences.getInstance();
    final extractData =json.decode(prefs.getString('userData')!) as Map<String, dynamic>;
    String token = extractData['token'];
      ...
      print("Response: ${ratingDetails}");
       if (response.statusCode == 200) {
        ...
        if (userDetails.isNotEmpty) {
          ...
          final reviews = userDetails.map((json) => RatingState.fromJson(json)).toList();
